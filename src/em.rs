...
pub struct EMConfig {
    ...
    /// Genome/transcript lengths for length normalization (genome -> length).
    pub genome_lengths: HashMap<String, usize>,
}
...
impl EMClassifier {
    ...
    fn initialize(&mut self, mappings: &ReadMappings) {
        ...
        // Initialize abundances from soft assignment sums (better than uniform)
        let mut init_sums: HashMap<String, f64> = HashMap::new();
        for (_, probs) in &self.soft_assignments {
            for (genome, prob) in probs {
                *init_sums.entry(genome.clone()).or_insert(0.0) += prob;
            }
        }

        let total: f64 = init_sums.values().sum();
        if total > 0.0 {
            for genome in &self.all_genomes {
                let abundance = init_sums.get(genome).copied().unwrap_or(0.0) / total;
                self.abundances.insert(genome.clone(), abundance.max(self.config.min_abundance));
            }
        } else {
            // Fallback to uniform
    ...
    /// M-step: Update theta_i = sum(P(genome_i | read_r)) / (total_reads * effective_length_i)
    fn m_step(&mut self) {
        ...
        // Normalize to get new abundances with length correction
        let new_abundances: HashMap<String, f64> = self
            .all_genomes
            .iter()
            .filter_map(|genome| {
                let sum_prob = sums.get(genome).copied().unwrap_or(0.0);
                let effective_len = self.config.genome_lengths
                    .get(genome)
                    .copied()
                    .unwrap_or(1000) as f64; // default 1000bp if unknown
                let abundance = sum_prob / (total_reads as f64 * effective_len);
